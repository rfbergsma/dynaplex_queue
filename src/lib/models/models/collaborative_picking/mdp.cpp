...
namespace DynaPlex::Models {
	namespace collaborative_picking
	{
		...
		void MDP::GetFeatures(const State& state, DynaPlex::Features& features)const {
			...
			if (!graph_feats){
				// feat 0: current picker id
				// feat [1, 6 * n_pickers + 1]: current_picker, picker_location, assigned_vehicle, vehicle_destination, distance_from_destination, picking_bool
				// feat [6* n_pickers + 1, n_feats]: vehicle_location, vehicle_destination, distance_from_destination, blocked_bool, picking_bool, drop_off_bool
