...
scaffolding with them.  It does, on the instance where those mechanisms
were dominant; the picture is more nuanced on the routing instance
(Section~\ref{sec:pe-anneal}), and that nuance is itself informative.
...
\subsection{The annealing dissociation: scaffolding is
problem-dependent, not obsolete}
\label{sec:pe-anneal}

The strong reading of Table~\ref{tab:pe} --- ``the reformulation makes
the guard obsolete'' --- does not survive Experiment~3.  Running the
anneal on/off comparison on both instances gives a clean double
dissociation (binary cost, 8 seeds within 5\% of RVI):

\begin{center}
\begin{tabular}{lcc}
\toprule
 & anneal ON & anneal OFF \\
\midrule
Experiment 2 (strategic idling) & 4/8 & \textbf{8/8} \\
Experiment 3 (specialist routing) & \textbf{7/8} & 3/8 \\
\bottomrule
\end{tabular}
\end{center}

The guard \emph{harms} Experiment 2 and \emph{helps} Experiment 3.  The
mechanism reading is consistent: Experiment 2 has no wrong-sign near-ties
left for annealing to fix (so sharpening only narrows the data), while
Experiment 3's routing structure still benefits from the behavior policy
converging toward the argmax during training.  The honest claim is
therefore not ``the scaffolding is obsolete'' but ``the scaffolding is
matched to a failure mode, and the per-event space removes that failure
mode on some instances but not all.''  Practically: a stabilizer should
be a switchable ingredient, not a fixed part of the recipe.

\subsection{All three objectives, both instances}
\label{sec:pe-complete}

With the objective-matched settings (binary: small batch; queue-lateness:
average-reward + large batch; fraction-late: escalation, below), the
per-event space solves every cell (Table~\ref{tab:pe-complete}).

\begin{table}[h]
\centering
\caption{Per-event PPO, seeds within 5\% of RVI, both instances.
Objective-matched recipes; no per-cell hyperparameter tuning.}
\label{tab:pe-complete}
\begin{tabular}{lcc}
\toprule
Objective & Experiment 2 & Experiment 3 \\
\midrule
Binary            & 8/8 (exact) & 7/8 \\
Queue-lateness    & 8/8 (exact) & 8/8 (exact) \\
Fraction-late (escalated) & 8/8 (exact) & 8/8 (exact) \\
\bottomrule
\end{tabular}
\end{table}

\subsection{The fraction-late objective and the escalation rule}
\label{sec:pe-escalation}

The fraction-served-late objective (\texttt{reward\_type}~4;
Section~\ref{sec:ql} derives its flux form) is the most business-relevant
target, but it is ill-posed on the FIL-projected model as stated: a
policy that lets a job rot suppresses that type's arrival stream in the
projection, so ``never serve late work'' becomes an artifact-optimum, and
RVI slowly discovers it (the benchmark drifts below achievable cost).

The fix is a modeling rule, not a reward hack: \emph{SLA escalation}.
When a capacity unit can serve a job already past its deadline, the
assignment is forced --- oldest-late-first, non-preemptive.  This is
interpretable as a real service rule (overdue work escalates to priority)
and is implemented purely as \texttt{IsAllowedAction} masking down to a
single allowed action, so the existing trivial-state machinery
auto-executes it: PPO, DCL, and RVI all solve the \emph{restricted} MDP
with zero algorithm changes, and the learned policy only ever decides
\emph{pre-deadline}.  Escalation makes the objective well-posed (late
states become transient, so RVI truncation converges at small $M$) and
kills the abandonment degeneracy by construction.  With it, learned
policies sit correctly \emph{above} RVI --- the earlier anomaly is gone.

A structural finding falls out of this objective:
\textbf{fraction-late headroom comes from routing topology, not
congestion.}  On the two-type instances (Experiments 2, 3) FIFO and c-$\mu$
are both optimal to within $0.01\%$ under escalation --- the objective is
trivially solvable there.  Headroom appears only on multi-hop routing
structures (below), which is why the two-type benchmarks understate the
difficulty of the objective.

\subsection{First scaling result: a three-type routing chain}
\label{sec:pe-scaling}

The natural scaling motivation for the per-event space --- $O(n)$
per-decision action set, event-local credit --- is tested on a serial
routing chain of three job types and three server pools (each pool serves
two adjacent types), under the escalated fraction-late objective at
moderate-high load.  This instance has genuine headroom: RVI $=1.121$,
FIFO $=1.34\times$ RVI, and c-$\mu$ $=2.07\times$ RVI (the myopic priority
rule is badly suboptimal, confirming a real routing problem rather than a
scheduling one).

Per-event PPO (large-batch average-reward recipe, no annealing) solves it
cleanly: all 8 seeds land in $[1.129, 1.151]$, i.e.\ within
$0.7$--$2.6\%$ of RVI, beating FIFO by $\sim16\%$ and c-$\mu$ by
$\sim2.05\times$.  The seed spread is $\sim2\%$ --- no lottery --- and all
values sit above RVI, confirming the escalated objective is well-posed at
this scale.

\paragraph{Four and six types.}
The same recipe, unchanged, was then run on a four-type chain and on the
six-type/five-pool instance, where RVI is intractable and policies are
judged against the heuristics (Table~\ref{tab:scaling}).  All 24 runs
beat the best heuristic on their cell, with no collapses and no lottery.

\begin{table}[h]
\centering
\caption{Scaling under the escalated fraction-late objective, 8 seeds per
cell, identical recipe.  ``Best heuristic'' is the stronger of FIFO and
c-$\mu$ on that cell.  RVI is available only on the three-type chain.}
\label{tab:scaling}
\begin{tabular}{lccccc}
\toprule
Instance & Types/pools & Best heuristic & PPO range & Gain & Spread \\
\midrule
frontier & 3/3 & FIFO $1.34$   & $1.129$--$1.151$ & $-15\%$ & $2.0\%$ \\
medium   & 4/4 & FIFO $2.05$   & $1.771$--$1.974$ & $-3$ to $-13\%$ & $11\%$ \\
large    & 6/5 & c-$\mu$ $42.40$ & $41.31$--$41.55$ & $-2.4$ to $-2.9\%$ & $0.6\%$ \\
\bottomrule
\end{tabular}
\end{table}

Two observations, one encouraging and one honest.  The encouraging one:
stability does \emph{not} degrade with instance size --- the six-type
cell has the \emph{smallest} seed spread ($0.6\%$) of the three, and
across all 24 runs there is no catastrophic seed.  The honest one: the
margin over the best heuristic shrinks with size ($-15\%$, $-13\%$,
$-2.9\%$).  A training-budget explanation was tested and rejected:
re-running the six-type cell at $2500$ updates ($2.5\times$ the budget)
gives $41.05$--$41.43$, inside the $1000$-update spread, so the policies
are converged and the small margin is structural, not a compute artifact.

The pattern is consistent with the topology finding of
Section~\ref{sec:pe-escalation}: what determines headroom is \emph{how
good the best heuristic happens to be on that structure}, not instance
size.  On the four-type chain c-$\mu$ is poor and PPO beats it by
$\sim1.9\times$; on the six-type instance c-$\mu$ is strong and only a
few percent remains.  The scaling claim supported by these data is
therefore: the method transfers to larger instances \emph{without
retuning, without instability, and at unchanged training cost}, and
extracts whatever headroom the instance contains --- verified against the
exact optimum where one is computable.
...
at its source, and the patches become unnecessary --- or, on the
strategic-idling instance, actively counterproductive --- while
remaining useful on the routing instance (Section~\ref{sec:pe-anneal}).
The reformulated space is the natural primary formulation for scaling:
its per-decision action set is independent of server count, its credit
assignment is event-local, it needs a strictly smaller recipe, and it
carries the most realistic objective to a three-type routing chain at
8/8 within a few percent of optimal.
